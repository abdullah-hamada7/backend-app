...
This is the backend part of the project. It's a Laravel application.
...
1. **Start the Server**:
    Run `vagrant up` in this folder. This starts the local server at `192.168.56.10`.

2. **Setup the Runner**:
    Go to your GitHub Repo > Settings > Actions > Runners and add a "Self-hosted runner" on your machine.

3. **Deploy**:
    Push to `main`. The workflow triggers your local runner, which uses Ansible to:
    * Pull the latest code.
    * Run `php artisan migrate` (automatically applying schema changes).
